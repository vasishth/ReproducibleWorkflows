...
But the above approach will take a lot of time, so for this demonstration, I compute the power for three values: the power estimates based on the estimated mean effect `r b1`. One could also compute power based on the lower `r b1-2*b1se`, and upper bounds `r b1+2*b1se` of the estimated effect.  
...
A better way to obtain estimates of the effect is by doing a meta-analysis. See @NicenboimPreactivation2019 for an estimate based on a meta-analysis. The estimate based on all publicly available data is: 0.11 microvolts, 95% credible intervals [0.05,0.16]. 
